...
According to the method for simulation described in @Lin1997, the true mean cost for data simulated with a uniform survival distribution is 40000, while the mean cost for exponentially distributed survival is 35956. Using the simCostData function to simulate date from these distributions with either light (~25%) or heavy (~40%) censoring we test the performance of the estimators in the ccostr package.

For a single simulation with n=1000 from a uniform survival distribution with light censoring we obtain the following results:
```{r}
...
We now test the bias and coverage of the estimators, by performing more extensive simulation. 
```{r, eval = FALSE}
set.seed(42)
nSim   <- 1000
nYears <- 10
...
For the calculation of the estimates we use parralization to speed up the process. We make an implicit cluster and use it for computations. Be aware, this might still take quite some time.
```{r, eval = FALSE}
...
### Bias
Subtracting the true mean from the estimates reveals the bias
```{r}
...
### Coverage
Similarly the overlap of confidence intervals with the true mean gives the coverage of the estimators
```{r}
